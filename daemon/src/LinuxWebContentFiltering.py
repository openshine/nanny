...
WEBDATABASE='/var/lib/nanny/webs.db'

def ipt(cmd) :
    return os.system("/sbin/iptables %s > /dev/null 2>&1" % cmd)

class LinuxWebContentFiltering(gobject.GObject) :
    def __init__(self, quarterback, app) :
        ...
        if os.path.exists('/var/lib/nanny/webs.db') :
